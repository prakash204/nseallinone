...
//   |   |   |   |   |   196.15.101.0  |   |   |   |   196.15.103.0   |    |    |     |
//  ===================               *   *   *   *                 ===================
//     196.15.100.0                     196.15.102.0                    196.15.104.0
...
main (int argc, char *argv[])
{
  ...
  csmaNodes1.Add (p2pNodes1.Get (0));//n5
  csmaNodes1.Create (4);// n1 n2 n3 n4
  ...
  address.SetBase ("196.15.101.0", "255.255.255.0");
  Ipv4InterfaceContainer p2pInterfaces1;
  p2pInterfaces1 = address.Assign (p2pDevices1);

  address.SetBase ("196.15.100.0", "255.255.255.0");
  Ipv4InterfaceContainer csmaInterfaces1;
  csmaInterfaces1 = address.Assign (csmaDevices1);

  address.SetBase ("196.15.102.0", "255.255.255.0");
  address.Assign (staDevices);
  address.Assign (apDevices);

  address.SetBase ("196.15.103.0", "255.255.255.0");
  Ipv4InterfaceContainer p2pInterfaces2;
  p2pInterfaces2 = address.Assign (p2pDevices2);

  address.SetBase ("196.15.104.0", "255.255.255.0");
  ...
  ApplicationContainer clientApps = 
   echoClient.Install (csmaNodes1.Get (4));
  clientApps.Start (Seconds (0.1));
  clientApps.Stop (Seconds (500.0));
  
  Ipv4GlobalRoutingHelper::PopulateRoutingTables ();
  
  csma.EnablePcap ("na2",csmaDevices2.Get(3),true);
